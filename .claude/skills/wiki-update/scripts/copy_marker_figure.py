...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--figure", required=True, type=int, help="Paper figure number")
...
def find_source_ref(markdown: str, figure_number: int, lookback_lines: int) -> str:
    lines = markdown.splitlines()
    caption_re = re.compile(rf"\bFigure\s+{figure_number}\s*:", re.IGNORECASE)
    for i, line in enumerate(lines):
        ...
        if candidates:
            return candidates[-1]
    raise SystemExit(f"figure {figure_number} image reference not found")
