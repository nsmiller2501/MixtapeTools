...
The parent runs install/cache/convert steps. If `<cache-dir>/text.md` exists, the parent copies it to `<text_path>` and skips extraction. Otherwise the parent prepares the extraction substrate, launches bounded workers, then runs one synthesis bottleneck.
...
5. Parent copies <text_path> to <cache-dir>/text.md for reuse by future projects using the same PDF hash.
